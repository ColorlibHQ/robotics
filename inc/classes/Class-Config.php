...
// Final Class
final class Robotics {

	// Theme Version
	private $robotics_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new Robotics_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->robotics_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'robotics_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'robotics', ROBOTICS_DIR_PATH . '/languages' );
		...
		add_image_size( 'robotics_blogsect_post_thumb', 295, 211, true );
		
		// Custom thumbnails size for widget post
		add_image_size( 'robotics_widget_post_thumb', 75, 75, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'robotics-np-thumb', 59, 59, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'robotics' ),
            'social-menu'   => esc_html__( 'Footer Social Menu', 'robotics' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = ROBOTICS_DIR_CSS_URI;
		$jsPath  = ROBOTICS_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					...
					'dependency' 	=> array(),
					'version' 		=> $this->robotics_version,
				),
				array(
					'handler'		=> 'robotics-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->robotics_version,
				),
				array(
					'handler'		=> 'robotics-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'robotics' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate robotics theme customizer
		$robotics_theme_customizer = new robotics_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'robotics_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'robotics_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'robotics-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'robotics_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'robotics-elementor-default-style',  ROBOTICS_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->robotics_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'robotics_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'robotics-elementor-notice', ROBOTICS_DIR_JS_URI.'robotics-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'robotics-elementor-notice',
				'roboticsElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'robotics-elementor-notice-nonce' ),
				)
			);
		}

...
function robotics_sl_enqueue_scripts() {
	...
	wp_enqueue_script( 'robotics-post-likes', ROBOTICS_COMPANION_DIR_URL .'/js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'robotics-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'robotics' )
	) );
}
...
function robotics_process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'robotics' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'robotics' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'robotics' );
		$icon = $icon_empty;
	}
...
function robotics_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'robotics' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'robotics' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
